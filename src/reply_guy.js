...
class XReplyGuy {
   ...
   */
  logToDashboard(tag, type, message) {
    try {
   ...
   */
  generateReplyText(kolHandle, tweetText) {
  ...
  async runReplyGuyTick(dryRun = false) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (isLocked && attempt < maxRetries) {
          console.log(`⚠️ [ReplyGuy Lock] Chrome Profile 正在被佔用，排隊排程中...（嘗試 ${attempt}/${maxRetries}，等待 8 秒後重試）`);
    ...
    try {
      ...
      // Generate reply text
      const replyData = this.generateReplyText(foundKol, foundTweet.text);
      ...
      console.error('[ReplyGuy Error]:', error.message);
      this.logToDashboard('流量衝刺', 'ERROR', `自動搶沙發失敗: ${error.message}`);
      return false;
    } finally {
      if (browser) {
        await browser.close();
        console.log('[ReplyGuy] Browser closed.');
      }
    }
  }
}
