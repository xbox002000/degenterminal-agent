...
🎁 Binance特典: ${binance}`;
  ...
  async getBestChartToAttach(stats) {
    ...
    // If no fresh trade, let's try to look for any pre-rendered chart in public/ to attach as asset
    try {
      const publicDir = path.join(__dirname, '../../../public');
      const files = fs.readdirSync(publicDir);
      const charts = files.filter(f => f.startsWith('chart_') && f.endsWith('.png'));
      if (charts.length > 0) {
        const chosen = charts[Math.floor(Math.random() * charts.length)];
