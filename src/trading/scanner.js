...
class OnChainScanner {
  ...
  async auditTokenComprehensive(token, pairData, rugData, mode = 'conservative') {
    const isBlueChip = [
      'JUPyiwrYd2Cfhw4GDgpQA8aa7a1yR91gCUZXjhbDiJF',
      'EKpQGSJtjMFqKZ9KQGWzeotVEhCoWkU1Y27acvZpump',
      'DezXAZ8z7PnrnRJjz3wXh3t1KCkbgCgX2j7fcx1a21wF',
      '7GCihJUkKEsHkM4TrcDZrR6GP549YmdPdHmkwwtgpump'
    ].includes(token.tokenAddress || token.address);

    if (isBlueChip) {
      console.log(`🛡️ [Scanner Blue-Chip Bypass] Detected established blue chip/ecosystem token $${token.header || 'Asset'}. Applying premium risk settings.`);
      const priceChange1h = pairData?.priceChange?.h1 || 0;
      // High volumeScore if it represents an oversold dip
      const blueChipVolumeScore = priceChange1h < -2 ? 98 : 88;

      return {
        compositeScore: 95,
        riskLevel: 'LOW',
        flags: ['[BLUE-CHIP] Premium Asset Bypass Active'],
        breakdown: {
          social: 100,
          liquidity: 100,
          volume: blueChipVolumeScore,
          contract: 100
        }
      };
    }
   ...
   * @param {number} fngValue - Current Fear & Greed index
   */
  async scanAndAudit(mode = 'conservative', fngValue = 50) {
    ...
    if (fngValue < 30) {
      console.log(`[Scanner] 🚨 Fear & Greed Index is at ${fngValue} (Fear). Activating Blue-Chip Meme Channel!`);
      const blueChips = [
        { address: 'JUPyiwrYd2Cfhw4GDgpQA8aa7a1yR91gCUZXjhbDiJF', symbol: 'JUP', name: 'Jupiter' },
        { address: 'EKpQGSJtjMFqKZ9KQGWzeotVEhCoWkU1Y27acvZpump', symbol: 'WIF', name: 'dogwifhat' },
        { address: 'DezXAZ8z7PnrnRJjz3wXh3t1KCkbgCgX2j7fcx1a21wF', symbol: 'BONK', name: 'Bonk' },
        { address: '7GCihJUkKEsHkM4TrcDZrR6GP549YmdPdHmkwwtgpump', symbol: 'POPCAT', name: 'Popcat' }
      ];
      blueChips.forEach(bc => {
        if (!candidates.some(c => c.tokenAddress === bc.address)) {
          candidates.unshift({
            chainId: 'solana',
            tokenAddress: bc.address,
            header: bc.name,
            symbol: bc.symbol,
            description: `Blue-Chip Ecosystem Asset: ${bc.name} ($${bc.symbol})`,
            links: [
              { type: 'website', url: `https://dexscreener.com/solana/${bc.address}` }
            ]
          });
          console.log(`🔥 [Scanner Blue-Chip Channel] Injected top tier token: $${bc.symbol} (${bc.address})`);
        }
      });
    }
    ...
    for (const token of tokensToProcess) {
      ...
      let symbol = token.symbol || token.tokenAddress.slice(0, 6).toUpperCase();

      if (name.startsWith('http') || name.includes('/') || name.includes('width=')) {
        name = `Token-${token.tokenAddress.slice(0, 6)}`;
        symbol = token.symbol || token.tokenAddress.slice(0, 6).toUpperCase();
      }
