...
async function testHeaded() {
  console.log('Spawning Chrome in HEADED mode (headless: false) via Junction...');
  
  const args = [
    '--remote-debugging-port=9222',
    // NO HEADLESS PARAMETERS AT ALL!
  ...
  try {
    ...
    console.log('SUCCESS: Logged in X via HEADED Junction! Compose textbox found!');
    
    // Take a screenshot
    const successPath = path.join(__dirname, 'x_headed_success.png');
    ...
    const failPath = path.join(__dirname, 'x_headed_fail.png');
