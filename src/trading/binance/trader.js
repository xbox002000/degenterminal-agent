...
class BinanceTrader {
   ...
   */
  async executeSpotOrder(symbol = 'BTCUSDT', side = 'BUY', usdtAmount = 500) {
    const uppercaseSymbol = symbol.toUpperCase();
    const uppercaseSide = side.toUpperCase();
    
    console.log(`\n--- 📈 [Binance Spot Testnet] 執行現貨下單: ${uppercaseSide} ${uppercaseSymbol} (金額: ${usdtAmount} USDT) ---`);

    if (!this.isConfigured()) {
      console.warn(`💡 [Binance Spot Testnet] API Key 或 Secret Key 未配置完全。啟用高仿真模擬紙盤模式...`);
      return this.simulateSpotOrder(uppercaseSymbol, uppercaseSide, usdtAmount);
    }
    ...
    try {
      ...
      if (uppercaseSide === 'BUY') {
        ...
        // Mock default quantity for sell if not passed
        const qty = (usdtAmount / 65000).toFixed(6);
      ...
      console.warn(`💡 [Binance Spot Testnet] 發生連線或金鑰錯誤，自動退回高仿真模擬紙盤完成閉環。`);
      return this.simulateSpotOrder(uppercaseSymbol, uppercaseSide, usdtAmount);
    }
  }
   ...
   */
  simulateSpotOrder(symbol, side, usdtAmount) {
    const mockOrderId = Math.floor(Math.random() * 10000000);
    const approxPrices = { BTCUSDT: 68500, SOLUSDT: 172, JUPUSDT: 1.05 };
   ...
   */
  async executeFuturesOrder(symbol = 'BTCUSDT', side = 'BUY', usdtAmount = 500, leverage = 5) {
    const uppercaseSymbol = symbol.toUpperCase();
    const uppercaseSide = side.toUpperCase();
    
    console.log(`\n--- 📈 [Binance Futures Testnet] 執行合約開開倉: ${uppercaseSide} ${uppercaseSymbol} (槓桿: ${leverage}x, 部位: ${usdtAmount} USDT) ---`);

    if (!this.isConfigured()) {
      console.warn(`💡 [Binance Futures Testnet] API Key 未配置完全。啟用高仿真模擬合約紙盤模式...`);
      return this.simulateFuturesOrder(uppercaseSymbol, uppercaseSide, usdtAmount, leverage);
    }
    ...
    try {
      ...
      // 2. Calculate quantity based on position size and index price
      // We first query the symbol price to calculate precision-aligned quantity
      ...
      // Format quantity precision (BTC has 3 decimals precision, SOL has 3, etc.)
      const qty = uppercaseSymbol.includes('BTC') ? rawQty.toFixed(3) : rawQty.toFixed(2);
      ...
      return {
        ...
        avgPrice: parseFloat(resData.avgPrice),
        rawResponse: resData
      };
      ...
      console.warn(`💡 [Binance Futures Testnet] 發生連線或金鑰錯誤，自動退回高仿真合約紙盤模擬。`);
      return this.simulateFuturesOrder(uppercaseSymbol, uppercaseSide, usdtAmount, leverage);
    }
  }
   ...
   */
  simulateFuturesOrder(symbol, side, usdtAmount, leverage) {
    const mockOrderId = Math.floor(Math.random() * 20000000);
    const approxPrices = { BTCUSDT: 68500, SOLUSDT: 172, JUPUSDT: 1.05 };
    const price = approxPrices[symbol] || 100;
    const rawQty = (usdtAmount * leverage) / price;
    const qty = symbol.includes('BTC') ? rawQty.toFixed(3) : rawQty.toFixed(2);

    console.log(`✅ [Simulated Binance Futures] 仿真期貨建倉成功！`);
    console.log(`• 訂單 ID: ${mockOrderId}`);
